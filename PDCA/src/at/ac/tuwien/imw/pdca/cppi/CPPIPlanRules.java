...
public class CPPIPlanRules implements PlanningRules<BigDecimal> {
	...
	public BigDecimal applyPlanningRules() {
		...
		log.info("Configuration period: " + service.getCurrentPeriod() + ", Floor: " + service.getCppiValues().getFloor().setScale(4, BigDecimal.ROUND_HALF_UP) + ", Cushion: " + service.getCppiValues().getCushion().setScale(4, BigDecimal.ROUND_HALF_UP) + ", Exposure: " + service.getCppiValues().getExposure().setScale(4, BigDecimal.ROUND_HALF_UP) + ", PartRisky: " + service.getCppiValues().getPartRiskyAsset().setScale(4, BigDecimal.ROUND_HALF_UP) + ", PartRiskless: " + service.getCppiValues().getPartRisklessAsset().setScale(4, BigDecimal.ROUND_HALF_UP) + ", NewPortfolio: " + service.getCppiValues().getPortfolio().setScale(4, BigDecimal.ROUND_HALF_UP) +
